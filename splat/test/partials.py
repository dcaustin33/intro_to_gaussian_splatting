...
def d_gaussian_strength_wrt_gaussian_weight(gaussian_weight: torch.Tensor) -> torch.Tensor:
    """
    input is a nx1 tensor
    output is a nx1 tensor
    """
...
def d_gaussian_weight_wrt_diff(diff: torch.Tensor, inv_covariance: torch.Tensor) -> torch.Tensor:
    """
...
class pixelSpaceToPixels(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        ...
        ctx.save_for_backward(width, height, intrinsic_matrix, extrinsic_matrix, points_3d)
        ...
        points_pixel_space = points_pixel_space_before_divide[:, :2] / points_pixel_space_before_divide[:, 3].unsqueeze(1)
    ...
    def backward(ctx, grad_output: torch.Tensor) -> torch.Tensor:
        ...
        ndc_before_divide = torch.cat([points_3d, torch.ones(points_3d.shape[0], 1)], dim=1) @ extrinsic_matrix @ intrinsic_matrix
        ...
        gradient = torch.bmm(grad_output.unsqueeze(1), derivative[:, :, :3]).squeeze(1)
...
class pixelCoordToColor(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.save_for_backward(color, inv_covariance_2d, current_T, opacity, gaussian_weight, diff)
        return color * current_T * alpha

    
    @staticmethod
    def backward(ctx, grad_output: torch.Tensor) -> torch.Tensor:
        color, inv_covariance_2d, current_T, opacity, gaussian_weight, diff = ctx.saved_tensors
        ...
        derivative = derivative * d_gaussian_strength_wrt_gaussian_weight(gaussian_weight).unsqueeze(1)
        ...
        deriviative = torch.bmm(grad_output.unsqueeze(1), derivative).squeeze(1)
        return None, deriviative, None, None, None, None, None