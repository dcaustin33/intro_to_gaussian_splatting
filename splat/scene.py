...
class GaussianScene(nn.Module):
    ...
    def get_points_and_covariance(
        ...
        camera_coords_x = final_points_in_camera_coords[:, 0]
        camera_coords_y = final_points_in_camera_coords[:, 1]
        camera_coords_z = final_points_in_camera_coords[:, 2]
        # n x 3 x 3
        jacobians = torch.zeros(
            (final_points_in_camera_coords.shape[0], 3, 3), device=points.device
        )
        jacobians[:, 0, 0] = f_x / camera_coords_z
        jacobians[:, 1, 1] = f_y / camera_coords_z
        jacobians[:, 0, 2] = -f_x * camera_coords_x / (camera_coords_z**2)
        jacobians[:, 1, 2] = -f_y * camera_coords_y / (camera_coords_z**2)
        # w is a 3x3 matrix
        T = torch.matmul(jacobians, W.T)
        projected_covariance = torch.matmul(
            torch.matmul(T, covariance_3d), T.transpose(1, 2)
        )[:, :2, :2]
        return projected_points, z_component, projected_covariance
